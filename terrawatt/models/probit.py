"""
Probit Model Module
Probit Regression implementation with spatial cross-validation using statsmodels.
"""
...
class ProbitModel(twm.InferenceModel):
    """Probit Regression Model using statsmodels."""

    def __init__(self, config_path="config.yaml"):
        """
        Initialize the Probit model.
    ...
    def _setup_logging(self):
        ...
        self.logger.info("Probit Model initialized")

    def _check_compatibility(self):
        """Check if the model kind is compatible."""
        model_cfg = self.config['model']
        if model_cfg['kind'] != self.kind:
            raise ValueError(f"Model kind '{model_cfg['kind']}' is not compatible with ProbitModel.")
    ...
    def _create_features(self):
        """
        Create features for Probit Regression.
    ...
    def _train(self, X=None, y=None):
        """
        Train the Probit model using statsmodels.
